...
export default function Playground() {
  ...
  useEffect(() => {
    ...
    if (startIndex < endIndex) {
      ...
      setDisplayedQuestions([
        ...sampleQuestions.slice(startIndex),
        ...sampleQuestions.slice(0, endIndex),
      ]);
    }
  }, [currentIndex]);
  ...
  return (
    <div className="flex flex-col items-center text-black justify-center min-h-screen bg-[#204544] m-0 p-0">
      ...
      {!graphState && (
        <>
          <div className="text-white text-center mb-20 w-2/3">
            Don't have a .sqlite or .csv file to query? We'll use this one by
            default:{" "}
            <a
      ...
      {graphState &&
        !(
          ...
          graphState.visualization == "none"
        ) && (
          <div className="flex  w-2/3  items-center justify-center mt-60">
      ...
      {graphState && graphState.visualization == "none" && (
        <div
          id="answer_canvas"
          className="p-10 w-2/3 flex flex-col items-center justify-center relative"
        >
          <button
          ...
          <div className="flex w-full flex-col p-10 rounded-[10px] bg-white items-center justify-center">
            ...
            {graphState.visualization_reason && (
              <div className="text-sm mt-10 text-gray-500 mx-20">
                {graphState.visualization_reason}
              </div>
            )}
          </div>
      ...
      {graphState && graphState.formatted_data_for_visualization && (
        <div
          id="answer_canvas"
          className="p-10 w-full flex flex-col items-center justify-center relative"
        >
          <button
            onClick={toggleSidebar}
            className="absolute top-12 right-12 bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded"
          ...
          <div className="flex w-full flex-col p-10 rounded-[10px] bg-white text-white items-center justify-center">
            <div className="text-sm mb-10 mx-20">
              {graphState.answer && (
                <div className=" text-black">{graphState.answer}</div>
              )}
            </div>
            {React.createElement(
              graphDictionary[
                graphState.visualization as keyof typeof graphDictionary
